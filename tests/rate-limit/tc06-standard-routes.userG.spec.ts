...
/**
 * TC-06: Standard Routes Rate Limit - User G (Endpoints 13-15)
 */

test.describe('TC-06: Standard Routes Rate Limit - User G (13-15)', () => {
  test.setTimeout(300000);

  const baseURL = process.env.API_BASE_URL || 'https://api-sit.askmebill.com';

  const credentials = {
    email: process.env.AUTH_EMAIL_G || 'eiji6',
  ...
  for (const endpoint of endpoints) {
    test(`TC-06: ${endpoint}`, async () => {
      ...
      const otherEndpoint = '/v2/md/billing-note/customer?status=REFUND&page=1&limit=25';
